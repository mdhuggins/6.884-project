...
from transformers import HfArgumentParser, PreTrainedTokenizerBase, BatchEncoding, TrainingArguments, BertConfig, \
    BertTokenizer, Trainer

import transformers

from transformers.trainer_utils import is_main_process, set_seed

from utils import get_retro_embeds
...
class CustomDataCollatorForLanguageModeling:
    ...
    def __call__(
        ...
        if self.mlm:
            ...
            batch["labels"] = labels
        retro_vecs = get_retro_embeds(batch["input_ids"],"../models/graphembeddings/numberbatch-en-19.08.txt")
        batch["retro_vecs"]=retro_vecs
        return batch
...
def main():
    ...
    model = LitOutputBertModel(name="test",config=config,
                               tokenizer=tokenizer,
    ...
    data_collator = CustomDataCollatorForLanguageModeling(tokenizer=tokenizer, mlm_probability=data_args.mlm_probability)
